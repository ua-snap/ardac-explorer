...
export const usePolygonsStore = defineStore('polygons', () => {
  ...
  async function fetchRandomPolygon() {
    ...
    try {
      ...
      const randomCategoryIndex = Math.floor(Math.random() * polygonCategories.length)
      ...
      if (!featuresResponse.ok) {
        throw new Error(`API request failed with status ${featuresResponse.status}`)
      }
  ...
  async function fetchFeatureGeometry(featureId: string) {
    try {
      ...
      if (!geometryResponse.ok) {
        throw new Error(`Geometry API request failed with status ${geometryResponse.status}`)
      }
